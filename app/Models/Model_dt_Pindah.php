...
class Model_dt_Pindah extends Model
{
    ...
    private function _get_datatables_query($bulan = null, $tahun = null, $kecamatan = null , $kelurahan = null)
    {
        ...
        if ($bulan != 'all') {
            $this->dt->where('EXTRACT(MONTH FROM tgl_pindah)', $bulan);
        }
        if ($tahun != 'all') {
            $this->dt->where('EXTRACT(YEAR FROM tgl_pindah)', $tahun);
        }
        if ($kecamatan != 'all') {
    ...
    {
        return $this->table('tb_data_pindah')
        ->where('YEAR(tgl_pindah)', $year)
        ->countAllResults();
    }
    ...
    //Dashboard - Grafik
    public function chart_pindah($tahun, $month, $kec, $kelamin)
    {
        return $this->table('tb_data_pindah')
            ->where('YEAR(tgl_pindah)', $tahun)
            ->where('MONTH(tgl_pindah)', $month)
            ->where('kecamatan', $kec)
            ->where('kelamin', $kelamin)
            ->countAllResults();
    }

    public function chart_kel_pindah($tahun, $month, $kel, $kelamin)
    {
        return $this->table('tb_data_pindah')
            ->where('YEAR(tgl_pindah)', $tahun)
            ->where('MONTH(tgl_pindah)', $month)
