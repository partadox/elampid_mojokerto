...
class PindahLaporan extends BaseController
{
    /*--- FRONT ---*/
	public function index()
	{
        ...
        return view('panel/pindah/laporan/index', $data);
	}
    ...
    public function export()
    {
        ...
        foreach ($kecamatan as $kec) {
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $query   = $this->db->query("SELECT COUNT(*) AS '$as' FROM tb_data_pindah WHERE EXTRACT(YEAR FROM tgl_pindah) = $tahun AND EXTRACT(MONTH FROM tgl_pindah) = $i AND kecamatan = '$kc'");
        ...
        foreach ($kelurahan as $kel) {
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $query   = $this->db->query("SELECT COUNT(*) AS '$as' FROM tb_data_pindah WHERE EXTRACT(YEAR FROM tgl_pindah) = $tahun AND EXTRACT(MONTH FROM tgl_pindah) = $i AND kelurahan = '$kl' AND kelamin = 'LAKI-LAKI'");
        ...
        foreach ($kelurahan as $kel) {
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $query   = $this->db->query("SELECT COUNT(*) AS '$as' FROM tb_data_pindah WHERE EXTRACT(YEAR FROM tgl_pindah) = $tahun AND EXTRACT(MONTH FROM tgl_pindah) = $i AND kelurahan = '$kl' AND kelamin = 'PEREMPUAN'");
        ...
        $judulT1 = "LAPORAN DATA PINDAH KOTA MOJOKERTO TAHUN " .$tahun;
        ...
        $filename =  'DATA PINDAH KOTA MOJOKERTO TAHUN '.$tahun.' - DIUNDUH TGL '. date('Y-m-d');
        if ($extension == 'xlsx') {
