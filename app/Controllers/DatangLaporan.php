...
class DatangLaporan extends BaseController
{
    /*--- FRONT ---*/
	public function index()
	{
        ...
        $data = [
            'title'         => 'Laporan Data Datang',
            'list_tahun'    => $list_tahun,
        ];
        return view('panel/datang/laporan/index', $data);
	}
    ...
    public function export()
    {
        ...
        foreach ($kecamatan as $kec) {
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $query   = $this->db->query("SELECT COUNT(*) AS '$as' FROM tb_data_datang WHERE EXTRACT(YEAR FROM tgl_datang) = $tahun AND EXTRACT(MONTH FROM tgl_datang) = $i AND kecamatan = '$kc'");
        ...
        foreach ($kelurahan as $kel) {
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $query   = $this->db->query("SELECT COUNT(*) AS '$as' FROM tb_data_datang WHERE EXTRACT(YEAR FROM tgl_datang) = $tahun AND EXTRACT(MONTH FROM tgl_datang) = $i AND kelurahan = '$kl' AND kelamin = 'LAKI-LAKI'");
        ...
        foreach ($kelurahan as $kel) {
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $query   = $this->db->query("SELECT COUNT(*) AS '$as' FROM tb_data_datang WHERE EXTRACT(YEAR FROM tgl_datang) = $tahun AND EXTRACT(MONTH FROM tgl_datang) = $i AND kelurahan = '$kl' AND kelamin = 'PEREMPUAN'");
        ...
        $judulT1 = "LAPORAN DATA DATANG KOTA MOJOKERTO TAHUN " .$tahun;
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('D12', $t2['L-gununggedangan-1'])
            ->setCellValue('E12', $t2['P-gununggedangan-1'])
            ->setCellValue('G12', $t2['L-gununggedangan-2'])
            ->setCellValue('H12', $t2['P-gununggedangan-2'])
            ->setCellValue('J12', $t2['L-gununggedangan-3'])
            ->setCellValue('K12', $t2['P-gununggedangan-3'])
            ->setCellValue('M12', $t2['L-gununggedangan-4'])
            ->setCellValue('N12', $t2['P-gununggedangan-4'])
            ->setCellValue('P12', $t2['L-gununggedangan-5'])
            ->setCellValue('Q12', $t2['P-gununggedangan-5'])
            ->setCellValue('S12', $t2['L-gununggedangan-6'])
            ->setCellValue('T12', $t2['P-gununggedangan-6'])
            ->setCellValue('V12', $t2['L-gununggedangan-7'])
            ->setCellValue('W12', $t2['P-gununggedangan-7'])
            ->setCellValue('Y12', $t2['L-gununggedangan-8'])
            ->setCellValue('Z12', $t2['P-gununggedangan-8'])
            ->setCellValue('AB12', $t2['L-gununggedangan-9'])
            ->setCellValue('AC12', $t2['P-gununggedangan-9'])
            ->setCellValue('AE12', $t2['L-gununggedangan-10'])
            ->setCellValue('AF12', $t2['P-gununggedangan-10'])
            ->setCellValue('AH12', $t2['L-gununggedangan-11'])
            ->setCellValue('AI12', $t2['P-gununggedangan-11'])
            ->setCellValue('AK12', $t2['L-gununggedangan-12'])
            ->setCellValue('AL12', $t2['P-gununggedangan-12'])
        ...
        $filename =  'DATA DATANG KOTA MOJOKERTO TAHUN '.$tahun.' - DIUNDUH TGL '. date('Y-m-d');
        if ($extension == 'xlsx') {
